...
_CITY_HINTS = {'北京', '上海', '天津', '重庆', '济南', '成都', '西安', '汉中', '洛阳', '南京', '合肥', '杭州', '郑州', '太原', '泰安', '曲阜'}
_ATTRACTION_HINTS = ('山', '寺', '庙', '馆', '宫', '园', '湖', '城墙', '石窟', '遗址', '古城', '古镇', '街', '塔', '三孔')


def _split_items(text: str) -> list[str]:
    return [item.strip() for item in re.split(r'[、,，和及]|再|然后|并且|以及', text) if item.strip()]


def _clean_item(item: str) -> str:
    cleaned = item.strip(' ，。；;！!?')
    cleaned = re.sub(r'^(先|再|然后|顺路|中途|沿途|路上|到|去|看|逛|安排|打卡|经过|途经|必须|一定要|不要遗漏|必打卡|必去)+', '', cleaned)
...
def extract_must_visit_attractions(text: str | None) -> list[str]:
    ...
    result: list[str] = []
    patterns = (
        r'(?:必须去|一定要安排|一定要去|不要遗漏|必打卡|必去)(?P<items>[^，。；;！!]+)',
        r'(?:中途想去|路上想去|沿途想看|中途想看)(?P<items>[^，。；;！!]+)',
        r'(?:到[^，。；;！!]{1,12}后必须去)(?P<items>[^，。；;！!]+)',
    )
...
def extract_waypoint_details(text: str | None) -> list[dict[str, Any]]:
    ...
    patterns = (
        ...
        (r'(?:必须去|一定要安排|一定要去|不要遗漏|必打卡|必去)(?P<items>[^，。；;！!]+)', True),
        (r'(?:中途想去|路上想去|沿途想看|中途想看)(?P<items>[^，。；;！!]+)', True),
        (r'(?:到[^，。；;！!]{1,12}后必须去)(?P<items>[^，。；;！!]+)', True),
    )
